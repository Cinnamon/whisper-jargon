...
from whisper_main.whisper.audio import SAMPLE_RATE, N_FRAMES, HOP_LENGTH, pad_or_trim, log_mel_spectrogram
...
class WhisperASRDataset(torch.utils.data.Dataset):
    def __init__(self,
        ...
        with open(f_manifest, newline='') as f_csv:
            ...
            for idx, row in enumerate(tqdm.tqdm(reader)):
                ...
                if (max_duration and duration > max_duration) or (min_duration and duration < min_duration):
                    continue
    ...
    def __getitem__(self, id):
        key, audio, transcript = self.manifest[id]["key"], self.manifest[id]["audio"], self.manifest[id]["transcript"]
